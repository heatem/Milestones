...
private func evaluate(_ node: String, _ attrs: [String: String?] = [:], _ c: Closure) {
    ...
    output = output + mergedAttributes.reduce("") { (result, item) in // added: (result, item) in
        if let value = item.value { // $0.1.1 changed to item.value
            return result + " \(item.key)=\"\(value)\"" // $0.0 changed to result, $0.1.0 changed to item.key
        } else {
            return result // $0.0 changed to result
        }
    }
